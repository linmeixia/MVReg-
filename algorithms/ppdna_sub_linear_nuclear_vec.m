function [U,beta,w,TtXtxi,Ztxi,xi,par,runhist,info] = ppdna_sub_linear_nuclear_vec(y,Ainput,B,gamma,XTB,Zgamma,TtXtxi,Ztxi,xi,para,par,options)
printsub = 1;
breakyes = 0;
maxitersub = 50;
tol = 1e-6;
...
%%
Uinput  = B - sig*TtXtxi;
[Up,info_U] = prox_nuclear_norm(Uinput,sig*rho,flag);
par.info_U = info_U;
U = (Uinput-Up)/sig;
...
if strcmp(para.vec_regularizer,'lasso')
    Lxi = Lxi + rho*norm(svd(Up),1) + lambda*sum(abs(betap)) + 0.5*norm(wp-y)^2;
elseif strcmp(para.vec_regularizer,'fused lasso')
    Bbetap = Binput.Bmap(betap);
    Lxi = Lxi + rho*norm(svd(Up),1) + lambda1*sum(abs(betap)) + lambda2*sum(abs(Bbetap)) + 0.5*norm(wp-y)^2;
elseif strcmp(para.vec_regularizer,'clustered lasso')
    betapsort = sort(betap,'descend');
    Lxi = Lxi + rho*norm(svd(Up),1) + lambda1*sum(abs(betap)) + lambda2*(wvec*betapsort) + 0.5*norm(wp-y)^2;
elseif strcmp(para.vec_regularizer,'sparse group lasso')
    Pbetap = P.matrix*(betap);
    Lxi = Lxi + rho*norm(svd(Up),1) + c(1)*sum(abs(betap)) + c(2)*P.Lasso_fz(Pbetap) + 0.5*norm(wp-y)^2;
elseif strcmp(para.vec_regularizer,'slope')
    Lxi = Lxi + rho*norm(svd(Up),1) + c_lambda'*sort(abs(betap),'descend') + 0.5*norm(wp-y)^2;
elseif strcmp(para.vec_regularizer,'exclusive lasso')
    Lxi = Lxi + rho*norm(svd(Up),1) + lambda1*xgroupnorm(betap,group_info) + 0.5*norm(wp-y)^2;
end
runhist.psqmr(1) = 0;
...
for itersub = 1:maxitersub
    ...
    psix = 0.5*norm(XUppZbetap-y)^2 + rho*norm(svd(Up),1) + norm(Up-B,'fro')^2/(2*sig) + norm(betap-gamma)^2/(2*sig) + norm(XUppZbetap-XTBpZgamma)^2/(2*sigdtau);
    if strcmp(para.vec_regularizer,'lasso')
    ...
    [dxi,resnrm,solve_ok,par] = ppdna_Netwonsolve_linear_nuclear_vec(Ainput,rhs,par);
