function [obj,B,gamma,s,w,U,beta,xi,info,runhist] = admm_linear_vecmat_vec(Xinput,Zinput,y,m,q,p,para,options,B0,gamma0,xi0,U0,beta0)
maxiter = 20000;
sigma   = 1;
paragamma   = 1.618;
stoptol = 1e-6;
...
if printyes
    ...
    if strcmp(Asolver,'cg')
        ...
        if printminoryes
            ...
            if strcmp(para.vec_regularizer,'lasso')
                fprintf('\n regularizers: vectorized L1 norm with rho = %g, lasso with lambda = %g', rho, lambda);
            elseif strcmp(para.vec_regularizer,'fused lasso')
                fprintf('\n regularizers: vectorized L1 norm with rho = %g, fused lasso with lambda1 = %g, lambda2 = %g', rho, lambda1, lambda2);
            elseif strcmp(para.vec_regularizer,'clustered lasso')
                fprintf('\n regularizers: vectorized L1 norm with rho = %g, clustered lasso with lambda1 = %g, lambda2 = %g', rho, lambda1, lambda2);
            elseif strcmp(para.vec_regularizer,'sparse group lasso')
                fprintf('\n regularizers: vectorized L1 norm with rho = %g, sparse group lasso with c1 = %g, c2 = %g', rho, c(1), c(2));
            elseif strcmp(para.vec_regularizer,'slope')
                fprintf('\n regularizers: vectorized L1 norm with rho = %g, slope with weights', rho);
            elseif strcmp(para.vec_regularizer,'exclusive lasso')
                fprintf('\n regularizers: vectorized L1 norm with rho = %g, exclusive lasso with lambda1 = %g', rho, lambda1);
            end
            fprintf('\n ---------------------------------------------------');
        end
...
for iter = 1:maxiter
    ...
    Up = reshape(proxL1(Uinput(:),sigma*rho),m,q);
    U = (Uinput -Up)/sigma;
    ...
    dualfeas = max([normRd1/(1+normU),normRd2/(1+normbeta),normRd3/(1+normxi)]);
    maxfeas = max(primfeas,dualfeas);
    proxpBpU = reshape(proxL1(B(:)+U(:),rho),m,q);
