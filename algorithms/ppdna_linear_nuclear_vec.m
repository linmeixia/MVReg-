function [obj,B,gamma,s,w,U,beta,xi,info,runhist] = ppdna_linear_nuclear_vec(Xinput,Zinput,y,m,q,p,para,options)
maxiter = 50;
stoptol = 1e-6;
precond = 0;
...
phaseI_stoptol = 1e-3;
phaseI_maxiter = 100;
startfun = @admm_linear_nuclear_vec;
admm.iter = 0;
admm.time = 0;
admm.timecpu = 0;
...
if admm.op.maxiter > 0 && runphaseI
    ...
    s = XTB + Zgamma;
    if strcmp(para.vec_regularizer,'lasso')
        obj(1) = 0.5*norm(s-y)^2 + rho*norm(svd(B),1) + lambda*sum(abs(gamma));
    elseif strcmp(para.vec_regularizer,'fused lasso')
        Bgamma = Binput.Bmap(gamma);
        obj(1) = 0.5*norm(s-y)^2 + rho*norm(svd(B),1) + lambda1*sum(abs(gamma)) + lambda2*sum(abs(Bgamma));
    elseif strcmp(para.vec_regularizer,'clustered lasso')
        gammasort = sort(gamma,'descend');
        obj(1) = 0.5*norm(s-y)^2 + rho*norm(svd(B),1) + lambda1*sum(abs(gamma)) + lambda2*(wvec*gammasort);
    elseif strcmp(para.vec_regularizer,'sparse group lasso')
        Pgamma = P.matrix*(gamma);
        obj(1) = 0.5*norm(s-y)^2 + rho*norm(svd(B),1) + c(1)*sum(abs(gamma)) + c(2)*P.Lasso_fz(Pgamma);
    elseif strcmp(para.vec_regularizer,'slope')
        obj(1) = 0.5*norm(s-y)^2 + rho*norm(svd(B),1) + c_lambda'*sort(abs(gamma),'descend');
    elseif strcmp(para.vec_regularizer,'exclusive lasso')
        obj(1) = 0.5*norm(s-y)^2 + rho*norm(svd(B),1) + lambda1*xgroupnorm(gamma,group_info);
    end
...
for iter = 1:maxiter
    ...
    [U,beta,w,TtXtxi,Ztxi,xi,parNCG,~,info_NCG] = ppdna_sub_linear_nuclear_vec(y,Ainput_nal,B,gamma,XTB,Zgamma,TtXtxi,Ztxi,xi,para,parNCG,ssncgop);
