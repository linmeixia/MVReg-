function [dxi,resnrm,solve_ok,par] = ppdna_Netwonsolve_linear_nuclear_vec(Ainput,rhs,par)
Z = Ainput.Z;
para = par.para;
...
sr = par.sigma*par.rho;
dS = par.info_U.dS;
alpha1 = par.info_U.alpha1;
alpha3 = par.info_U.alpha3;
par.t13 = (dS(alpha1)-sr)./(dS(alpha1)-dS(alpha3)');
par.w = (dS(alpha1)-sr+max(dS'-sr,0))./(dS(alpha1)+dS');

V = par.info_U.V;
if par.flag == 1
    par.V1 = V(:,1:par.m);
    par.V2 = V(:,par.m+1:par.q);
    par.mu = (dS(alpha1)-sr)./dS(alpha1);
elseif par.flag == 2
    par.V1 = V;
    par.V2 = [];
else
    par.V1 = V(:,1:par.q);
    par.V2 = V(:,par.q+1:par.m);
    par.mu = (dS(alpha1)-sr)./dS(alpha1);
end
%% iterative solver
if par.precond == 1
...
[dxi,~,resnrm,solve_ok] = psqmry('mat_ppdna_linear_nuclear_vec',0,rhs,par);%numblk1
end
